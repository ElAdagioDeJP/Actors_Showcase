/*fetch('https://freetestapi.com/api/v1/actors')
    ...
    document.addEventListener('DOMContentLoaded', () => { // Espera a que el DOM esté completamente cargado antes de ejecutar el código
        ...
        searchInput.addEventListener('input', (event) => { // Añade un evento de escucha para detectar cambios en el campo de búsqueda
    ...
    class APIService {
        constructor(urlApi) { // Constructor que inicializa la URL de la API
            this.urlApi = urlApi; // Asigna la URL de la API a una propiedad de la instancia
        }
    
        obtenerActorPorId(id) { // Método para obtener un actor por su ID
    ...
    class ActorManager {
        constructor(apiService) { // Constructor que inicializa el servicio de API
        ...
        actualizarActores(nuevoActor) { // Método para actualizar la lista de actores con un nuevo actor
        ...
        eliminarActor(id) { // Método para eliminar un actor por su ID
            this.actores = this.actores.reduce((result, actor) => // Filtra el array de actores para excluir el actor con el ID dado
                actor.id !== id ? result.concat(actor) : result, []); // Si el ID del actor no coincide, lo añade al resultado
            this.idsCargados.delete(id); // Elimina el ID del conjunto de IDs cargados
        }
    
        obtenerActoresDisponibles() { // Método para obtener la lista de actores disponibles
            return this.actores; // Retorna el array de actores
        }
    
        buscarActoresPorNombre(nombre) { // Método para buscar actores por nombre
            return this.actores.reduce((result, actor) => // Filtra el array de actores para incluir solo aquellos cuyo nombre coincide con la búsqueda
                actor.name.toLowerCase().includes(nombre.toLowerCase()) ? result.concat(actor) : result, []); // Si el nombre del actor incluye el término de búsqueda, lo añade al resultado
        }
    }
    
    class TablaRenderer {
        constructor(actorManager) { // Constructor que inicializa el gestor de actores
        ...
        renderizarTabla(actores) { // Método para renderizar la tabla con una lista de actores
        ...
        crearFila(actor) { // Método para crear una fila de la tabla para un actor
            const fila = document.createElement('tr'); // Crea un elemento de fila
    
            const crearCelda = contenido => { // Función para crear una celda de la tabla
            ...
            fila.append( // Añade las celdas a la fila
        ...
        actualizarConteoActores(conteo) { // Método para actualizar el conteo de actores
            this.elementoConteoActores.textContent = conteo; // Asigna el conteo de actores al elemento de conteo
        }
    }
    
    class TablaActores {
        constructor(urlApi) { // Constructor que inicializa la URL de la API y otras propiedades
        ...
        inicializar() { // Método para inicializar la tabla de actores
            window.addEventListener('load', () => { // Añade un evento de carga a la ventana
                this.programarActualizacion(); // Llama al método programarActualizacion
            });
        }
    
        programarActualizacion() { // Método para programar la actualización de actores
        ...
        actualizarActorAleatorio() { // Método para actualizar un actor aleatorio
            ...
            actorId !== null && ( // Si el ID no es nulo
                ...
                this.apiService.obtenerActorPorId(actorId).then(actor => { // Obtiene el actor por su ID
        ...
        obtenerIdAleatorio() { // Método para obtener un ID aleatorio
            const idsPosibles = Array.from({ length: 40 }, (_, i) => i + 1); // Crea un array de IDs posibles del 1 al 40
            const idsDisponibles = idsPosibles.reduce((result, id) => // Filtra los IDs disponibles
                !this.idsGenerados.includes(id) ? result.concat(id) : result, []); // Si el ID no está en los IDs generados, lo añade al resultado
            const nuevoId = idsDisponibles.length > 0 ? idsDisponibles[Math.floor(Math.random() * idsDisponibles.length)] : null; // Selecciona un ID aleatorio de los disponibles
        ...
        verActor(id) { // Método para ver los detalles de un actor
            const actor = this.actorManager.actores.find(actor => actor.id === id); // Encuentra el actor por su ID
            alert(`Nombre: ${actor.name}\nPremios: ${actor.awards.join(', ')}\nDetalles: ${JSON.stringify(actor)}`); // Muestra los detalles del actor en una alerta
        }
    
        eliminarActor(id) { // Método para eliminar un actor por su ID
            this.actorManager.eliminarActor(id); // Elimina el actor del gestor de actores
            this.idsGenerados = this.idsGenerados.reduce((result, generatedId) => // Filtra los IDs generados para excluir el ID eliminado
                generatedId !== id ? result.concat(generatedId) : result, []); // Si el ID generado no coincide, lo añade al resultado
            this.guardarIdsGenerados(); // Guarda los IDs generados en localStorage
            this.guardarActores(); // Guarda los actores en localStorage
            console.log(`ID eliminado: ${id}`); // Imprime el ID eliminado en la consola
            console.log(`IDs guardados después de eliminar: ${this.idsGenerados.join(', ')}`); // Imprime los IDs guardados después de eliminar en la consola
            this.renderizarTabla(); // Renderiza la tabla con los actores disponibles o filtrados
            this.programarActualizacion(); // Programa la siguiente actualización después de 5 segundos
        }
    
        buscarActorPorNombre(nombre) { // Método para buscar actores por nombre
        ...
        renderizarTabla() { // Método para renderizar la tabla
        ...
        guardarIdsGenerados() { // Método para guardar los IDs generados en localStorage
        ...
        cargarIdsGenerados() { // Método para cargar los IDs generados desde localStorage
        ...
        guardarActores() { // Método para guardar los actores en localStorage
            localStorage.setItem('actores', JSON.stringify(this.actorManager.obtenerActoresDisponibles())); // Guarda los actores disponibles en localStorage
        }
    
        cargarActores() { // Método para cargar los actores desde localStorage
            const actoresGuardados = JSON.parse(localStorage.getItem('actores') || '[]'); // Carga los actores guardados desde localStorage o un array vacío si no hay ninguno
