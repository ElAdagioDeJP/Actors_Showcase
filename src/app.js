class TablaActores {
  ...
  inicializar() {
    ...
    this.verGrafico(); // Muestra el gráfico de densidades
  ...
  obtenerActores() {
    fetch(this.urlApi)
      ...
      .then((data) => {
        const actoresDisponibles = data.filter(
          (actor) => !this.idsActoresEliminados.has(actor.id)
        ); // Filtra los actores eliminados
        const actoresAleatorios = this.obtenerActoresAleatorios(
          actoresDisponibles,
          40
        ); // Obtiene 40 actores aleatorios
  ...
  obtenerActorAleatorio() {
    return fetch(this.urlApi)
      ...
      .then((data) => {
        ...
        const actorAleatorio = this.obtenerActoresAleatorios(
          actoresDisponibles,
          1
        )[0]; // Obtiene un actor aleatorio
  ...
  renderizarTabla() {
    const actoresFiltrados = this.actores.filter(
      (actor) => actor.name.toLowerCase().includes(this.consultaActual) // Filtra los actores según la consulta de búsqueda
    );
    this.cuerpoTabla.innerHTML = ""; // Limpia el cuerpo de la tabla
    actoresFiltrados.forEach((actor) =>
      this.cuerpoTabla.appendChild(this.crearFila(actor))
    ); // Agrega filas para cada actor
  ...
  verActor(id) {
    ...
    alert(
      `Nombre: ${actor.name}\nPremios: ${actor.awards.join(
        ", "
      )}\nDetalles: ${JSON.stringify(actor)}`
    ); // Muestra una alerta con los detalles del actor
  }
  ...
  rellenarActores() {
    ...
    const promesasFetch = Array.from({ length: cantidadRelleno }, () =>
      this.obtenerActorAleatorio()
    ); // Crea un array de promesas para obtener actores aleatorios
  ...
  configurarBusqueda() {
    this.inputBusqueda.addEventListener(
      "input",
      this.actualizarResultadosBusqueda.bind(this)
    ); // Agrega un evento de input al input de búsqueda
  }
